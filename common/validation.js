...
function validateGallery(request, response, next)
{
    ...
    if(request.file)
        request.checkBody("image", "Archivo incompatible, usa formato: 'jpeg' o 'png'").isImageValid(request.file.mimetype);
    request.checkBody("description", "La descripción de la imagen es obligatoria").notEmpty();
    request.checkBody("image", "No tienes suficientes puntos para subir una imagen").hasEnoughPoints(response.locals.currentPoints);
